...
def main() -> int:
    ...
    parser.add_argument(
        "username",
        type=str,
        help="Your bandcamp username, as it appears at the end of your bandcamp collection url, I.E. bandcamp.com/user_name",
    )
    parser.add_argument(
        "--browser",
        "-b",
    ...
    if CONFIG["VERBOSE"]:
        print(
            "Found [{}] links for [{}]'s collection.".format(
...
def download_album(_album_url: str, _attempt: int = 1) -> None:
    try:
        ...
        track_info = {key: data["download_items"][0][key]
...
def download_file(_url: str, _track_info: dict = None, _attempt: int = 1) -> None:
    try:
        ...
        if _attempt < CONFIG["MAX_URL_ATTEMPTS"]:
            ...
            print_exception(
                e,
                "An exception occurred trying to download file url [{}]:".format(
                    _url),
            )
    except Exception as e:
        print_exception(
            e, "An exception occurred trying to download file url [{}]:".format(
                _url)
        )
